...
void test2pic(const char* filename1, const char* filename2, bool bIsShowImg = true, bool bIsOriginFeatures = false, bool bIsEulDist = false, bool bIsUseDNNFeatures = true)
{
	...
	anet_type dnn_net;
	deserialize("dlib_face_recognition_resnet_model_v1.dat") >> dnn_net;
	cout << "load dnn net success." << endl;
	...
	if (bIsUseDNNFeatures)
	{
		cout << "computing image 1 DNN face features..." << endl;
		vf1 = GetAllFaceFeaturesByDNN(mat1, dnn_net, shapes1);
		cout << "computing image 2 DNN face features..." << endl;
		vf2 = GetAllFaceFeaturesByDNN(mat2, dnn_net, shapes2);
	}
	else
	{
...
int main(int argc, char *argv[])
{
	...
	if (argc < 3)
	{
		cout << "plz input two photos with human faces." << endl;
		return -1;
	}

	if (!QFile::exists(argv[1]) || !QFile::exists(argv[2]))
	{
		cout << "input file is not exist. Plz check them." << endl;
		return -2;
	}
	...
	//bIsOriginFeatures 是否使用原始绝对特征/相对特征
	//bIsEulDist 是否使用欧式距离/余弦距离
	//bIsUseDNNFeatures 是否使用深度学习特征
	test2pic(argv[1], argv[2], true, false, true, true);
	cv::waitKey(0);
	return 1;
