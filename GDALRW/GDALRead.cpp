...
double CGDALRead::linearRead(double _row, double _col, size_t _band)
{
	double dval = -9999;
	switch (mgDataType)
	{
	case GDT_Byte:
		dval = linRead<unsigned char>(_row, _col, _band);
		break;
	case GDT_UInt16:
		dval = linRead<unsigned short>(_row, _col, _band);
		break;
	case GDT_Int16:
		dval = linRead<short>(_row, _col, _band);
		break;
	case GDT_UInt32:
		dval = linRead<unsigned int>(_row, _col, _band);
		break;
	case GDT_Int32:
		dval = linRead<int>(_row, _col, _band);
		break;
	case GDT_Float32:
		dval = linRead<float>(_row, _col, _band);
		break;
	case GDT_Float64:
		dval = linRead<double>(_row, _col, _band);
		break;
	default:
		cout << "GDALRead: Unknown Datatype. Return -9999." << endl;
		dval = -9999;
		break;
	}
...
template<class TT> double CGDALRead::linRead( double _row, double _col, size_t _band )
{
	...
	float xpos = _row - long(_row);
	float ypos = _col - long(_col);

	//get the pixel value of 4-neighbour
	tx1 = _row+1.0; ty1 = _col+1.0;

	val[0] =*(TT*)readL(long(_row), long(_col), _band);	//band
	val[1] =*(TT*)readL(long(tx1), long(_col), _band);
	val[2] =*(TT*)readL(long(_row), long(ty1), _band);
	val[3] =*(TT*)readL(long(tx1), long(ty1), _band);
