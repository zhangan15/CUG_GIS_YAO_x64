...
//#include "memory_handler.h"
 ...
 int _lss_h12(int mode, int lpivot, int l1, int m, double *u, int iue,
              double *up, double *cm, int ice, int icv, int ncv);


 void _lss_g1(double a, double b, double *cterm, double *sterm, double *sig);

 int _lss_hfti(double* A, int m_a, int m, int n_a, int n, double* B, int m_b, int n_b, 
               double tau, int* krank, double* rnorm, double* w1, 
               double* w2, int* ip);

 void _lss_print_matrix(const double* M, int m, int n);
 void _lss_print_matrix2(const double* M, int m, int n, int icv, int ice);
 void _lss_matrix_times_vector( const double* M, int m, int n, int m_t, int n_t, const double* v,
                               int ie, double* x);
